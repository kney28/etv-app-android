        ...
        <q-card-section>
          <q-form ref="myForm" @submit.prevent="">
            <div class="row justify-around">
              ...
              <div class="col-md-4 col-sm-6 col-xs-12">
                <q-select
                  v-model="caracterInstitucional"
                  label="Caracter institucional *"
              ...
              <div class="col-md-4 col-sm-6 col-xs-12">
                <q-input white color="blue" type="number" v-model="personas_protegidas" label="# de personas protegidas *" lazy-rules
                  :rules=" [val => !!val || 'El campo es obligatorio'] " />
              </div>
              <div class="col-md-4 col-sm-6 col-xs-12">
              ...
              <div class="col-md-4 col-sm-6 col-xs-12">
                <q-select
                  v-model="tipo_criadero"
                  label="Tipo criadero o foco *"
                  :options="tipo_criaderoOpt"
                  option-value="id"
...
export default defineComponent({
  ...
  setup () {
    ...
    const razon_social = ref(null)
    const telefono = ref(null)
    const caracterInstitucional = ref(null)
    const publica = ref(null)
    const privada = ref(null)
    const categoria = ref(null)
    const personas_protegidas = ref(null)
    const representante_legal = ref(null)
    const cedula = ref(null)
    const criadero_cantidad = ref(null)
    const tipo_criadero = ref(null)
    const tipo_criaderoOpt = ref(TIPOCRIADERO)
    const criadero_ubicacion = ref(null)
    const criadero_potencial_cantidad = ref(null)
    const tipo_criadero_potencial = ref(null)
    const criadero_potencial_ubicacion = ref(null)
    const observaciones = ref(null)
    const toldillo_adulto_bueno = ref(null)
    const toldillo_adulto_regular = ref(null)
    const toldillo_adulto_malo = ref(null)
    const toldillo_adulto_total = ref(null)
    const toldillo_adulto_uso = ref(null)
    const toldillo_pedriatico_bueno = ref(null)
    const toldillo_pedriatico_regular = ref(null)
    const toldillo_pedriatico_malo = ref(null)
    const toldillo_pedriatico_total = ref(null)
    const toldillo_pedriatico_uso = ref(null)
    const toldillo_observaciones = ref(null)
    const role = ref(null)
    ...
    const onDelete = (row) => {
      $q.dialog({
        title: 'Confirmaci�n',
        message: '�Est� seguro que desea eliminar la etv_fv-ev-04: ' + row.id + '?',
        ok: {
          label: 'Si',
          color: 'positive'
        },
