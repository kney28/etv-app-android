  ...
  <q-page class="q-pa-md q-gutter-sm">
            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                ...
                <q-td key="formato" :props="props" class="text-left">
                  <q-btn size="sm" flat padding="none" text-color="blue-10" :label="props.row.id_form"
                    @click="showFormat(props.row)" />
                </q-td>
    ...
    <q-dialog v-model="dialog" persistent position="top">
      <q-card style="max-width: 90vw; max-height: 80vh" class="q-mt-md">
        ...
        <q-card-section class="q-pa-none q-pt-sm card-content">
            ...
            <div style="max-width: 100vw;">
              ...
              <q-tab-panels keep-alive v-model="tab" animated>
                <q-tab-panel name="section1">
                  ...
                  <div class="col-md-4 col-sm-6 col-xs-12">
                    <q-input label="Fecha" v-model="fecha" mask="date" readonly>
                      <template v-slot:append>
                  ...
                  <div class="col-md-4 col-sm-6 col-xs-12">
                    <q-select use-input v-model="municipio" label="Municipio *" @filter="filterMunicipios"
                      :options="optMunicipios" option-label="nom_municipio" option-value="cod_municipio" emit-value
                      map-options lazy-rules :rules="[val => !!val || 'El campo es obligatorio']"
                      @update:model-value="limpiarSelect('establecimiento')" />
                  </div>
                  <div class="col-md-4 col-sm-6 col-xs-12">
                    <q-select use-input v-model="establecimiento" label="Establecimiento *" @filter="filterEntidades"
                      :options="optEntidades" option-label="nom_establecimiento" option-value="doc_establecimiento"
                      emit-value map-options lazy-rules :rules="[val => !!val || 'El campo es obligatorio']"
                      @update:model-value="limpiarSelect('direccion')" />
                  </div>
                  <div class="col-md-4 col-sm-6 col-xs-12">
                    <q-select use-input v-model="direccion" label="Dirección *" @filter="filterDirecciones"
                      :options="optDirecciones" option-label="direccion" option-value="id" emit-value map-options
                ...
                <q-tab-panel name="section2">
                  ...
                  <q-card-section class="row items-center q-gutter-sm">
                    <q-btn class="q-mt-sm" color="secondary" @click="addFieldSet(1)" icon="add" label="Agregar"
                      :disable="criaderos.length == 10" />
                    <div>Criaderos: {{ criaderos.length }}</div>
                  </q-card-section>
                  <q-card-section>
                ...
                <q-tab-panel name="section3">
                        ...
                        <div class="field-set">
                          ...
                          <div class="col-md-4 col-sm-6 col-xs-12">
                            <q-select v-model="criaderoPotencial.tipo" label="Tipo criadero o foco *"
                              :options="tipo_criaderoOpt" lazy-rules
                              :rules="[val => !!val || 'El campo es obligatorio']" />
                          </div>
                          <div class="col-md-4 col-sm-6 col-xs-12">
                  ...
                  <div class="col-md-4 col-sm-6 col-xs-12">
                    <q-input white color="blue" type="textarea" v-model="observaciones" label="Observaciones *"
                  ...
                  <div :class="$q.dark.mode ? 'q-mt-sm q-pa-sm' : 'bg-grey-2 q-mt-sm q-pa-sm'">
                    ...
                    <div class="col-md-4 col-sm-6 col-xs-12">
                      <q-input white color="blue" type="number" v-model="toldillo_adulto_regular"
                        label="Estado regular (No.) *" lazy-rules
                        :rules="[val => !!val || 'El campo es obligatorio']" />
                    </div>
                    <div class="col-md-4 col-sm-6 col-xs-12">
                      <q-input white color="blue" type="number" v-model="toldillo_adulto_malo"
                    ...
                    <div class="col-md-4 col-sm-6 col-xs-12">
                      <q-input white color="blue" type="number" v-model="toldillo_adulto_uso" label="En Uso *"
                        lazy-rules
...
import { TIPOCRIADERO, saveFile } from 'src/constants/constants'
...
export default defineComponent({
  ...
  setup() {
    ...
    const criaderos = ref([
      { cantidad: '5', tipo: 'tina', ubicacion: 'Patio' },
      { cantidad: '5', tipo: 'tina', ubicacion: 'sala' },
      { cantidad: '5', tipo: 'tina', ubicacion: 'Patio' },
      { cantidad: '5', tipo: 'tina', ubicacion: 'Patio' },
      { cantidad: '5', tipo: 'tina', ubicacion: 'Patio' },
      { cantidad: '5', tipo: 'tina', ubicacion: 'Patio' },
    ...
    const criaderosPotencial = ref([
      { cantidad: '5', tipo: 'tina', ubicacion: 'Patio' },
      { cantidad: '10', tipo: 'tarros_baldes', ubicacion: 'sala' },
      { cantidad: '21', tipo: 'tina', ubicacion: 'Patio' },
      { cantidad: '15', tipo: 'tina', ubicacion: 'Patio' },
      { cantidad: '5', tipo: 'tina', ubicacion: 'Patio' },
      { cantidad: '3', tipo: 'tarros_baldes', ubicacion: 'Patio' },
      { cantidad: '5', tipo: 'tina', ubicacion: 'Patio' },
      { cantidad: '5', tipo: 'tina', ubicacion: 'Patio' },
      { cantidad: '2', tipo: 'tarros_baldes', ubicacion: 'Patio' },
      { cantidad: '1', tipo: 'tina', ubicacion: 'Patio' },
    ])
    ...
    watch(() => municipio.value, async (val) => {
      ...
      // dataEntidades.value = [{ nom_establecimiento: 'Los Embalses', doc_establecimiento: '875520145' }, { nom_establecimiento: 'Los Corrales', doc_establecimiento: '875520116' }]
      await db.close()
    ...
    watch(() => establecimiento.value, async (val) => {
      ...
      // dataDirecciones.value = [{ direccion: 'Calle 4N #24-21', doc_establecimiento: '875520145' }, { direccion: 'cra 3 #4-58', doc_establecimiento: '875520116' }]
      await db.close()
    ...
    const showFormat = (row) => {
      message.value = row.json_data
      console.log(row.json_data)
      dialogWarning.value = true
    }

    const flattenAspects = (aspectos, tipo = 'encontrado') => {
      const aspectosLineales = {}
      const values = aspectos.map((item) => {
        if (tipo == 'encontrado') {
          return {
            CriaderoEncontradoCant: item.cantidad,
            CriaderoEncontradoTipo: item.tipo,
            CriaderoEncontradoUbicacion: item.ubicacion
          }
        }
        if (tipo == 'potencial') {
          return {
            CriaderoPotencialCant: item.cantidad,
            CriaderoPotencialTipo: item.tipo,
            CriaderoPotencialUbicacion: item.ubicacion
          }
        }
      })
      ...
      Object.keys(values).forEach((key) => {
        ...
        if (typeof value === 'object' && value !== null) {
          ...
          Object.keys(value).forEach((subKey) => {
            ...
            aspectosLineales[`${subKey}_${number}`] = value[subKey]
          })
        } else {
      ...
      myForm.value.validate().then(async success => {
        if (success) {
          ...
          const data = {
            ...
            //cedula: cedula.value,
            ...flattenAspects(criaderos.value),
            ...flattenAspects(criaderosPotencial.value, 'potencial'),
