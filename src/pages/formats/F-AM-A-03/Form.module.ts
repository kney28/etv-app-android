...
import { useFAMA03 } from 'src/stores/F-AM-A-03'
...
export class Form extends GeneralForms implements FormActions {
  ...
  setStores() {
    this.fama03 = useFAMA03()
    this.generalities = useGeneralities()
    this.entity = useEntityIdentification()
    this.aspects = [
      () => this.validateAspect(this.fama03.aspect1),
      () => this.validateAspect(this.fama03.aspect2),
      () => this.validateAspect(this.fama03.aspect3),
      () => this.validateAspect(this.fama03.aspect4),
      () => this.validateAspect(this.fama03.aspect5),
      () => this.validateAspect(this.fama03.aspect6),
      () => this.validateSanitaryMeasure(this.generalities),
      () => this.validateReasonVisit(this.generalities),
      () => this.validateSanitaryRequirements(this.generalities, this.fama03),
    ]
  }
  ...
  getAspectsAndScores(): Record<string, any> {
    ...
    aspectos.forEach((aspecto, i) => {
      this.fama03[aspecto].forEach((item: { cr: boolean, value: number | null, observations: null | string, na: boolean }, index: number) => {
        aspectosYClasificaciones[`${aspecto}.${index + 1}`] = {
          cr: item.cr, value: item.value, observations: item.observations, na: item?.na ?? false
        }
      })
      aspectosYClasificaciones[`clasificacionBlq${i + 1}`] = this.fama03.scoreAspect(aspecto)
    })
  ...
  async save(userName: string, GPS: { [key: string]: any }): Promise<DataInsert> {
    ...
    return {
      id: uid(),
      identificadorEntrevista: 'F-AM-A-03',
      ...
      puntajeConceptoSanitario: this.fama03.score,
      conceptoSanitario: this.fama03.emitConcept,
  ...
  resetValues(): void {
    this.fama03.resetValues()
    this.generalities.resetValues()
    this.entity.resetValues()
    console.log('resetValues FAMA03')
  }
  ...
  resetStore(): void {
     ...
     */
    const hasPropertys = Object.keys(this.fama03).length > 0
  ...
  validateSection2() {
    ...
    for (let i = 1; i <= count; i++) {
      result = result && this.validateAspect(this.fama03[`aspect${i}`]).status
    }
