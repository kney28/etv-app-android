...
export class Form extends GeneralForms implements FormActions {
  ...
  setStores() {
    this.fama07 = useFAMA07()
    this.generalities = useGeneralities()
    this.entity = useEntityIdentification()
    this.aspects = [
      () => this.validateAspect(this.fama07.aspect1),
      () => this.validateAspect(this.fama07.aspect2),
      () => this.validateAspect(this.fama07.aspect3),
      () => this.validateAspect(this.fama07.aspect4),
      () => this.validateAspect(this.fama07.aspect5),
  ...
  getAspectsAndScores(): Record<string, any> {
    ...
    aspectos.forEach((aspecto, i) => {
      this.fama07[aspecto].forEach((item: { cr: boolean, value: number | null, observations: null | string, na: boolean }, index: number) => {
        aspectosYClasificaciones[`${aspecto}.${index + 1}`] = {
          cr: item.cr, value: item.value, observations: item.observations, na: item?.na ?? false
        }
      })
      aspectosYClasificaciones[`clasificacionBlq${i + 1}`] = this.fama07.scoreAspect(aspecto)
    })
  ...
  async save(userName: string, GPS: { [key: string]: any }): Promise<DataInsert> {
    ...
    return {
      id: uid(),
      identificadorEntrevista: 'F-AM-A-07',
      ...
      puntajeConceptoSanitario: this.fama07.score,
      conceptoSanitario: this.fama07.emitConcept,
  ...
  resetValues(): void {
    this.fama07.resetValues()
    this.generalities.resetValues()
    this.entity.resetValues()
    console.log('resetValues FAMA07')
  }
  ...
  resetStore(): void {
     ...
     */
    const hasPropertys = Object.keys(this.fama07).length > 0
  ...
  validateSection2() {
    ...
    for (let i = 1; i <= count; i++) {
      result = result && this.validateAspect(this.fama07[`aspect${i}`]).status
    }
