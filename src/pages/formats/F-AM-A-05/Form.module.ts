...
export class Form extends GeneralForms implements FormActions {
  ...
  setStores() {
    this.fama05 = useFAMA05()
    this.generalities = useGeneralities()
    this.entity = useEntityIdentification()
    this.aspects = [
      () => this.validateAspect(this.fama05.aspect1),
      () => this.validateAspect(this.fama05.aspect2),
      () => this.validateAspect(this.fama05.aspect3),
      () => this.validateAspect(this.fama05.aspect4),
      () => this.validateAspect(this.fama05.aspect5),
  ...
  getAspectsAndScores(): Record<string, any> {
    ...
    aspectos.forEach((aspecto, i) => {
      this.fama05[aspecto].forEach((item: { cr: boolean, value: number | null, observations: null | string, na: boolean }, index: number) => {
        aspectosYClasificaciones[`${aspecto}.${index + 1}`] = {
          cr: item.cr, value: item.value, observations: item.observations, na: item?.na ?? false
        }
      })
      aspectosYClasificaciones[`clasificacionBlq${i + 1}`] = this.fama05.scoreAspect(aspecto)
    })
  ...
  async save(userName: string, GPS: { [key: string]: any }): Promise<DataInsert> {
    ...
    return {
      id: uid(),
      identificadorEntrevista: 'F-AM-A-05',
      ...
      puntajeConceptoSanitario: this.fama05.score,
      conceptoSanitario: this.fama05.emitConcept,
  ...
  resetValues(): void {
    this.fama05.resetValues()
    this.generalities.resetValues()
    this.entity.resetValues()
    console.log('resetValues FAMA05')
  }
  ...
  resetStore(): void {
     ...
     */
    const hasPropertys = Object.keys(this.fama05).length > 0
  ...
  validateSection2() {
    ...
    for (let i = 1; i <= count; i++) {
      result = result && this.validateAspect(this.fama05[`aspect${i}`]).status
    }
