...
import { useFAMA04 } from 'src/stores/F-AM-A-04'
...
export class Form extends GeneralForms implements FormActions {
  ...
  setStores() {
    this.fama04 = useFAMA04()
    this.generalities = useGeneralities()
    this.entity = useEntityIdentification()
    this.aspects = [
      () => this.validateAspect(this.fama04.aspect1),
      () => this.validateAspect(this.fama04.aspect2),
      () => this.validateAspect(this.fama04.aspect3),
      () => this.validateAspect(this.fama04.aspect4),
      () => this.validateAspect(this.fama04.aspect5),
      () => this.validateAspect(this.fama04.aspect6),
  ...
  getAspectsAndScores(): Record<string, any> {
    ...
    aspectos.forEach((aspecto, i) => {
      this.fama04[aspecto].forEach((item: { cr: boolean, value: number | null, observations: null | string, na: boolean }, index: number) => {
        aspectosYClasificaciones[`${aspecto}.${index + 1}`] = {
          cr: item.cr, value: item.value, observations: item.observations, na: item?.na ?? false
        }
      })
      aspectosYClasificaciones[`clasificacionBlq${i + 1}`] = this.fama04.scoreAspect(aspecto)
    })
  ...
  async save(userName: string, GPS: { [key: string]: any }): Promise<DataInsert> {
    ...
    return {
      id: uid(),
      identificadorEntrevista: 'F-AM-A-04',
      ...
      puntajeConceptoSanitario: this.fama04.score,
      conceptoSanitario: this.fama04.emitConcept,
  ...
  resetValues(): void {
    this.fama04.resetValues()
    this.generalities.resetValues()
    this.entity.resetValues()
    console.log('resetValues FAMA04')
  }
  ...
  resetStore(): void {
     ...
     */
    const hasPropertys = Object.keys(this.fama04).length > 0
  ...
  validateSection2() {
    ...
    for (let i = 1; i <= count; i++) {
      result = result && this.validateAspect(this.fama04[`aspect${i}`]).status
    }
