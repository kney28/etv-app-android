<template>
  ...
  <k-evaluation :items="aspect4A" @action="onAction" />
</template>
...
export default defineComponent({
  ...
  emits: ['action'],
  setup() {
    const { ['aspect4.1']: aspect4A } = storeToRefs(useFAMA29())
    const principal = usePrincipal()
    principal.currentForm = 4

    const onAction = (val) => {
      if (val.type === 'na') {
        aspect4A.value[val.data.index].value = aspect4A.value[val.data.index].options[0].val
      }
    }
