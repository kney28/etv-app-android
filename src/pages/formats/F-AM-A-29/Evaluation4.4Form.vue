<template>
  ...
  <k-evaluation :items="aspect4D" @action="onAction" />
</template>
...
export default defineComponent({
  ...
  emits: ['action'],
  setup() {
    const { ['aspect4.4']: aspect4D } = storeToRefs(useFAMA29())
    const principal = usePrincipal()
    principal.currentForm = 7

    const onAction = (val) => {
      if (val.type === 'na') {
        aspect4D.value[val.data.index].value = aspect4D.value[val.data.index].options[0].val
      }
    }
