            ...
            <template v-slot:body="props">
              <q-tr :props="props">
                <q-td key="formato" :props="props">
                  <q-btn size="sm" flat text-color="blue-10" :label="props.row.formato" @click="showFormat(props.row)" />
                </q-td>
  ...
  setup() {
    ...
    /*const dataEntidades = computed(() => {
      if (municipio.value) {
        ...
        //let result = await dbGlobal.query(`SELECT * FROM establecimientos WHERE cod_municipio = ${municipio.value}`)
